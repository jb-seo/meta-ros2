# Generated by superflore -- DO NOT EDIT
#
# Copyright 2019 Open Source Robotics Foundation
# Distributed under the terms of the BSD license

DESCRIPTION = "This contains CvBridge, which converts between ROS2     Image messages and OpenCV images."
AUTHOR = "Ethan Gao <ethan.gao@linux.intel.com>"
HOMEPAGE = "http://www.ros.org/wiki/cv_bridge"
SECTION = "devel"
LICENSE = "BSD"
LIC_FILES_CHKSUM = "file://package.xml;beginline=11;endline=11;md5=d566ef916e9dedc494f5f793a6690ba5"

ROS_BUILD_DEPENDS = "ament-index-python sensor-msgs boost opencv python3-numpy"
DEPENDS = "${ROS_BUILD_DEPENDS}"

ROS_BUILDTOOL_DEPENDS = "ament-cmake-ros-native python-cmake-module-native"
DEPENDS += "${ROS_BUILDTOOL_DEPENDS}"

# Currently informational only -- see http://www.ros.org/reps/rep-0149.html#dependency-tags.
ROS_TEST_DEPENDS = "ament-cmake-gtest-native ament-cmake-pytest-native ament-lint-auto-native ament-lint-common-native python3-opencv"

SRC_URI = "https://github.com/ros2-gbp/vision_opencv-release/archive/release/bouncy/cv_bridge/2.0.5-0.tar.gz;downloadfilename=${ROS_SP}.tar.gz"
SRC_URI[md5sum] = "3367ac0b470d8625a70fa17201f6ded2"
SRC_URI[sha256sum] = "a2ec0f5b7b3b3c966c0fbc7c488922b15d7f931961350be9d1bbe105e54d27e3"
S = "${WORKDIR}/vision_opencv-release-release-bouncy-cv_bridge-2.0.5-0"

ROS_BUILD_TYPE = "ament_cmake"
ROS_RECIPES_TREE = "recipes-ros2"

# Allow the above settings to be overridden.
include ${ROS_LAYERDIR}/recipes-ros/cv-bridge/cv-bridge-common.inc
include ${ROS_LAYERDIR}/recipes-ros2/cv-bridge/cv-bridge-common.inc
include ${ROS_LAYERDIR}/${ROS_RECIPES_TREE}/cv-bridge/cv-bridge-common-${PV}.inc
include ${ROS_LAYERDIR}/${ROS_RECIPES_TREE}/cv-bridge/${BPN}.inc
include ${ROS_LAYERDIR}/${ROS_RECIPES_TREE}/cv-bridge/${BPN}-${PV}.inc

inherit ros_bouncy
inherit ros_${ROS_BUILD_TYPE}

